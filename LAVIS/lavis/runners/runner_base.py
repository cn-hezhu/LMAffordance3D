            ...
            if len(self.valid_splits) > 0 and (self.evaluate_only or cur_epoch%self.val_freq == 0):
                for split_name in self.valid_splits:
                    ...
                    if val_log is not None:
                        if is_main_process():
                            assert (
                                "auc_metric" in val_log
                            ), "No auc_metric found in validation log."

                            auc_metric = val_log["auc_metric"]
                            prediction = val_log["prediction"]
                            if auc_metric > best_auc_metric and split_name == "val":
                                best_epoch, best_auc_metric = cur_epoch, auc_metric
                                output_file = os.path.join(self.output_dir, "prediction_best.txt")
                                np.savetxt(output_file, prediction, fmt='%.6f', delimiter=' ')
                                if not self.evaluate_only:
                                    self._save_checkpoint(cur_epoch, is_best=True)

                            del val_log["prediction"]
